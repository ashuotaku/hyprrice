...
class ContextMenuFix {
    ...
    isNavMenuExpanded(buttonWrapper) {
        const menu = buttonWrapper?.querySelector?.(
            '[class*="menu-container"]',
        );
        if (!menu) return false;
        const inner =
            menu.querySelector?.('[class*="nav-menu-container"]') || menu;
    ...
    findSeasonDropdown(element) {
        ...
        return (
            el.querySelector?.(
                '[role="listbox"][class*="open"], [class*="dropdown"][class*="open"]',
            ) || null
        );
    }
    ...
    isSeasonDropdownExpanded(dropdown) {
        ...
        const hasOptions = dropdown.querySelector?.(
            '[class*="option-"], .option-HcOSE',
        );
    ...
    checkAndFixSeasonDropdown(element) {
        ...
        if (
            dropdown.closest(
                '[class*="settings"], [class*="preferences"], [class*="select-input"], [class*="multiselect"]',
            )
        )
            return;
        ...
        const tryMove = (attempt = 0) => {
            if (
                ...
                dropdown.closest(".context-menu-portal")
            )
                return;
            if (this.seasonDropdownProcessed.has(dropdown)) return;
    ...
    checkAndFixContextMenu(element) {
        ...
        const isContextMenu =
            element.classList?.contains("menu-container-B6cqK") ||
        ...
        const isNavMenu =
            menuContainer.querySelector?.(
                '.user-info-container-uigVE, [class*="user-info-container"]',
            ) ||
            menuContainer.querySelector?.(
                '.nav-menu-section-j87xd, [class*="nav-menu-section"]',
            ) ||
            menuContainer.querySelector?.(
                '.nav-menu-container-Pl25j, [class*="nav-menu-container"]',
            );

        // Problematic container (side drawer, player, etc.)
        const problematicParent = menuContainer.closest(
            '.side-drawer-r9EuA, [class*="side-drawer"], .series-content-VkYHB, [class*="series-content"], .player-container-wIELK, [class*="player-container"], .theater-container, .player-video',
        );
        ...
        if (isNavMenu || problematicParent) {
            ...
            if (asNav) {
                const buttonWrapper =
                    menuContainer.closest(
                        '.menu-button-container-DtW4v, [class*="menu-button-container"], .nav-menu-popup-label-XmUBo, [class*="nav-menu-popup-label"], .label-container-XOyzm',
                    ) || menuContainer.parentElement;
                if (
                    !buttonWrapper?.isConnected ||
                    buttonWrapper.closest(".context-menu-portal") ||
                    this.navMenuProcessed.has(buttonWrapper)
                )
                    return;
                ...
                requestAnimationFrame(() =>
                    requestAnimationFrame(() => tryMove(0)),
                );
            } else {
    ...
    moveNavMenuToBody(buttonWrapper, menuContainer) {
        ...
        const menuInClone = buttonClone.querySelector(
            '[class*="menu-container"]',
        );
        if (menuInClone) {
            const inner =
                menuInClone.querySelector(
                    '.nav-menu-container-Pl25j, [class*="nav-menu-container"]',
                ) || menuInClone;
        ...
        const optionSelectors =
            '[class*="nav-menu-option-container"], [class*="logout-button-container"]';
        ...
        if (originalParent)
            removalObserver.observe(originalParent, {
                childList: true,
                subtree: true,
            });
    ...
    moveMenuToBody(menuContainer, isNavMenu = false) {
        ...
        const options = menuClone.querySelectorAll(
            '[class*="context-menu-option-container"]',
        );
        options.forEach((option) => {
            option.style.pointerEvents = "auto";
            option.addEventListener("click", (e) => {
                const originalOptions = menuContainer.querySelectorAll(
                    '[class*="context-menu-option-container"]',
                );
    ...
    destroy() {
        ...
        document
            .querySelectorAll(".context-menu-portal")
